...
// alert('new dfa code');
var sc_code_ver="1.25"
//1.25 - New version to include the integration code for dfa 2.0
...
s.linkInternalFilters="javascript:,saksfifthavenue"
...
s.usePlugins=true
s.variableProvider = 'DFA#1515780:v32=[["DFA-"+lis+"-"+lip+"-"+lastimp+"-"+lastimptime+"-"+lcs+"-"+lcp+"-"+lastclk+"-"+lastclktime]]';
s._channelPattern="International|DFA_INT>International|360i INT>International|CSE INT>International|AFF INT>Shopping Comparison|CSE>Shopping Comparison|>Shopping Comparison|GGLBASE001>Shopping Comparison|BZR002>Shopping Comparison|SHOP002>Shopping Comparison|PGR002>Shopping Comparison|SORTPRICE01>Shopping Comparison|SD001>Shopping Comparison|MSNLIVE001>Shopping Comparison|NEX002>Shopping Comparison|MSNSHOP001>Shopping Comparison|360iFroogle>Shopping Comparison|SDF001>Shopping Comparison|PRONTO01>Shopping Comparison|LIKE01>Shopping Comparison|yah300>Shopping Comparison|YAH301>Shopping Comparison|GIFTS01>Shopping Comparison|BZR001>Shopping Comparison|PGR001>Shopping Comparison|GGLPRADS001>Shopping Comparison|AMZN001>Email|EML>AOL|AOL>Affiliates|AFF>eBilling|EBILL>Brand Links|VEND>Search|SEA>Search|MSN>Search|360i>Banner Click|MED>Banner Click|DFA>Social Media|FCBK>Social Media|TWEET>Social Media|SOC"
s.prop33=sc_code_ver;
...
var dfaConfig = {
	CSID:               '1515780', // DFA Client Site ID
	SPOTID:             '1602125', // DFA Spotlight ID
	tEvar:              'eVar32', // Transfer variable, typically the "View Through" eVar.
	errorEvar:          undefined, // DFA error tracking (optional)
...
function s_doPlugins(s) {
	...
	if(s.ActionDepthTest) s.pdvalue=s.getActionDepth("s_depth");
	if(s.pageName!="emailpop:form") {
		s.channelManager('site_refer,kw_refer,dfaid','','','','s_direct');
		if (s._channel=="Natural Search") {
			if(s._keywords && s._keywords!="") s.campaign = s._partner + ':' + s._keywords;
		}
		if(myDfaFlag&&myDfaFlag!='1'&&myDfaFlag!="") s._channel="Banner Click"
		s.eVar49=s._channel;
		//>> #190
		// track timestamp of clients from incoming marketing channels or clients
		// starting a new session.
		if (s._channel.length || s.c_r('s_ev54') != '1') {
			var now = new Date();
			s.eVar54 = s._channel.length ? now.offsetTime(now.edt() ? -4 : -5).format("ddmmmyyyy:HH:MM:ss").toUpperCase() : "";
			s.c_w('s_ev54', '1');
			// stack the referring timestamp variable
			s.eVar55=s.crossVisitParticipation(s.eVar54,'s_ev55','30','5','>','purchase');
		}
		//<< #190
		if(s.ActionDepthTest && s.pdvalue == 1 && (!s.eVar49 || s.eVar49 == "")) {
			s.eVar49="Direct Load";
		}
        s.getPreviousValue(s.eVar49,'s_v49','');
	} else if(s.pdvalue == 2) {
		s.eVar49=s.getPreviousValue(s.eVar49,'s_v49','');
	}
	s.eVar50=s.crossVisitParticipation(s.eVar49,'s_ev50','30','5','>','purchase');  // stack the referring type variable
	...
	s.eVar3 = s.getValOnce(s.eVar3, 's_eVar3');


    if(s.getQueryParam('brandname')){
		s.prop14=s.getQueryParam('brandname');
        s.eVar14=s.prop14;
	}

	if(s.getQueryParam('EmailUpdateString')){
        s.eVar13=s.getQueryParam('EmailUpdateString');
	}

	s.eVar19=s.getQueryParam('ev19');

	// Override HTTP referrer if applicable
	if(!s.referrer){
	...
	if(s.events.indexOf('purchase')>-1)
  	    s.eVar7='stop';
 	s.eVar7=s.getTimeToComplete(s.eVar7,'ttc',0);


 	/* s.crossVisitParticipation Example: 1.2 */
    s.eVar11 = s.crossVisitParticipation(s.campaign,'s_cpm','90','5','>','purchase');
    ...
    s.prop8=s.getTimeParting('h','-5',Set_Year()); // Set hour
    s.prop9=s.getTimeParting('d','-5',Set_Year()); // Set day
    s.prop10=s.getTimeParting('w','-5',Set_Year()); // Set Weekend / Weekday
    s.eVar8=s.getTimeParting('h','-5',Set_Year()); // Set hour
    s.eVar9=s.getTimeParting('d','-5',Set_Year()); // Set day
    s.eVar10=s.getTimeParting('w','-5',Set_Year()); // Set Weekend / Weekday

    s.prop11=s.trackRefresh(s.pageName,"tr_PageRefresh");
...
s.visitorNamespace="saksfifthavenue"
s.dc=122
s.vmk="498DC5AD"
s.trackingServer="sitectlyst.saksfifthavenue.com"
s.trackingServerSecure="ssitectlyst.saksfifthavenue.com"
